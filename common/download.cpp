...
void common_download_run_tasks(const std::vector<common_download_task> & tasks) {
    for (const auto & task : tasks) {
        const int status = common_download_file_single(task.url, task.local_path, task.opts, task.is_hf);
        if (status < 200 || status >= 400) {
            throw std::runtime_error("download is unavailable in this build (no cpp-httplib): " + task.url);
        }
    }
}
...
common_download_hf_plan common_download_get_hf_plan(
