...
void params_translator::apply_extra_json(common_params & params, const json & extra) {
    ...
    if (extra.contains("cache_ram_mib"))              params.cache_ram_mib                = extra.at("cache_ram_mib").get<int32_t>();
    if (extra.contains("cache_idle_slots"))           params.cache_idle_slots             = extra.at("cache_idle_slots").get<bool>();
    ...
    if (extra.contains("mtmd_batch_max_tokens"))      params.mtmd_batch_max_tokens        = extra.at("mtmd_batch_max_tokens").get<int>();
    ...
    // MTP drafting knobs. Only the speculative.draft fields are exposed
    // here; the on/off switch and draft model path live on the typed config.
    // Unknown keys are ignored on purpose, same convention as the rest of
    // apply_extra_json.
    if (extra.contains("mtp") && extra.at("mtp").is_object()) {
        const auto & mtp = extra.at("mtp");
        if (mtp.contains("n_max"))   params.speculative.draft.n_max   = mtp.at("n_max").get<int32_t>();
        if (mtp.contains("n_min"))   params.speculative.draft.n_min   = mtp.at("n_min").get<int32_t>();
        if (mtp.contains("p_min"))   params.speculative.draft.p_min   = mtp.at("p_min").get<float>();
        if (mtp.contains("p_split")) params.speculative.draft.p_split = mtp.at("p_split").get<float>();
    }
}
...
common_params params_translator::translate(const llama_engine_config & cfg) {
    ...
    // Upstream now lets the MTP/draft context choose its KV cache type from
    // speculative.draft.*. Keep LlamaEngine's single KV knob applying to both
    // target and MTP contexts unless a dedicated draft KV option is added.
    ...
    // When enabled we make `types` contain *exactly* DRAFT_MTP; upstream
    // explicitly disables RS-based MTP in combination with other spec types
    // (see `server : disable RS-based MTP in combination with other spec
    // types`). If a draft model path is given, it overrides self-MTP.
    if (cfg.enable_mtp) {
        p.speculative.types = { COMMON_SPECULATIVE_TYPE_DRAFT_MTP };
        if (cfg.mtp_draft_model_path != nullptr && cfg.mtp_draft_model_path[0] != '\0') {
            p.speculative.draft.mparams.path = cfg.mtp_draft_model_path;
        }
    }
